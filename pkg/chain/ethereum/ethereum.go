...
func (bc *baseChain) GetBlockNumberByTimestamp(
	...
	block, err := bc.currentBlockHeader()
	if err != nil {
	...
	for block.Time > timestamp {
		...
		block, err = bc.headerByNumber(block.Number.Uint64() - blockDiff)
		if err != nil {
	...
	for block.Time < timestamp {
		block, err = bc.headerByNumber(block.Number.Uint64() + 1)
		if err != nil {
			return 0, fmt.Errorf("cannot get block: [%v]", err)
		}
	}
	...
	if block.Time > timestamp {
		previousBlock, err := bc.headerByNumber(block.Number.Uint64() - 1)
		if err != nil {
			return 0, fmt.Errorf("cannot get block: [%v]", err)
		}
