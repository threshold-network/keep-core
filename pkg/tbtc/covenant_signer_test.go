...
// controllableBlockCounter is a chain.BlockCounter whose height is set
// explicitly by the test. Swapping it into node.chain lets certificate-expiry
// checks (which read node.chain.BlockCounter().CurrentBlock()) be driven
// deterministically, independently of the wall-clock-driven local counter that
// the already-created signing executor uses during threshold signing.
type controllableBlockCounter struct {
	mutex  sync.Mutex
	height uint64
}

func (c *controllableBlockCounter) setHeight(height uint64) {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	c.height = height
}

func (c *controllableBlockCounter) CurrentBlock() (uint64, error) {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	return c.height, nil
}

func (c *controllableBlockCounter) WaitForBlockHeight(uint64) error {
	return nil
}

func (c *controllableBlockCounter) BlockHeightWaiter(blockNumber uint64) (<-chan uint64, error) {
...
func (c *controllableBlockCounter) WatchBlocks(ctx context.Context) <-chan uint64 {
...
// buildValidSelfV1CovenantRequest builds a fully valid self_v1 covenant signing
// request, including a genuinely threshold-signed v2 signer approval certificate
// with an EndBlock of testSignerApprovalEndBlock. It mirrors the setup exercised
// by TestCovenantSignerEngine_SubmitSelfV1Ready so expiry-focused tests reach
// the certificate check with everything else already valid.
func buildValidSelfV1CovenantRequest(
	t *testing.T,
	node *node,
	bitcoinChain *localBitcoinChain,
	walletPublicKey *ecdsa.PublicKey,
	...
	migrationDestination := &covenantsigner.MigrationDestinationReservation{
		ReservationID: "cmdr_self_expiry",
		Reserve:       reserve,
		Epoch:         12,
		Route:         covenantsigner.ReservationRouteMigration,
		Revealer:      revealer,
	...
	request := covenantsigner.RouteSubmitRequest{
		FacadeRequestID:           "rf_self_expiry",
		IdempotencyKey:            "idem_self_expiry",
		RequestType:               covenantsigner.RequestTypeReconstruct,
		Route:                     covenantsigner.TemplateSelfV1,
		Strategy:                  "0x1234",
		Reserve:                   reserve,
...
// TestCovenantSignerEngineCurrentBlockHeightMatchesHostChain asserts that the
// real engine reads the current height from the host chain's block counter.
func TestCovenantSignerEngineCurrentBlockHeightMatchesHostChain(t *testing.T) {
	node, _, _ := setupCovenantSignerTestNode(t)

	blockCounter := &controllableBlockCounter{}
	blockCounter.setHeight(4242)
	node.chain.(*localChain).blockCounter = blockCounter

	engine := &covenantSignerEngine{node: node}
	height, err := engine.CurrentBlockHeight(context.Background())
	if err != nil {
		t.Fatal(err)
	}

	hostChainCounter, err := node.chain.BlockCounter()
	if err != nil {
		t.Fatal(err)
	}
	expected, err := hostChainCounter.CurrentBlock()
	if err != nil {
		t.Fatal(err)
	}

	if height != expected {
		t.Fatalf("expected current block height %d, got %d", expected, height)
	}
}

// TestCovenantSignerEngineSubmitRejectsExpiredV2Certificate submits a genuinely
// threshold-signed v2 certificate after advancing the host chain past its signed
// EndBlock and asserts Submit rejects it before covenant transaction signing.
func TestCovenantSignerEngineSubmitRejectsExpiredV2Certificate(t *testing.T) {
	node, bitcoinChain, walletPublicKey := setupCovenantSignerTestNode(t)
	...
	// Issue the certificate with the wall-clock counter still in place so
	// threshold signing (which waits on block advancement) progresses normally.
	request := buildValidSelfV1CovenantRequest(t, node, bitcoinChain, walletPublicKey)

	// Freeze the host chain one block past the certificate's signed expiry. This
	// is only read by the expiry check; Submit is rejected before OnSubmit, so no
	// covenant signing (and thus no block waiting) happens against this counter.
	blockCounter := &controllableBlockCounter{}
	blockCounter.setHeight(testSignerApprovalEndBlock + 1)
	node.chain.(*localChain).blockCounter = blockCounter

	_, err = service.Submit(context.Background(), covenantsigner.TemplateSelfV1, covenantsigner.SignerSubmitInput{
		RouteRequestID: "ors_self_expired",
		Stage:          covenantsigner.StageSignerCoordination,
		Request:        request,
	})
	if err == nil || !strings.Contains(err.Error(), "signer approval certificate has expired") {
		t.Fatalf("expected expiration rejection before covenant signing, got %v", err)
	}
}

// TestCovenantSignerEnginePollRejectsExpiredV2Certificate submits while the
// certificate is still valid, advances the host chain past its signed EndBlock,
// and asserts Poll then rejects it through the real engine/provider path.
func TestCovenantSignerEnginePollRejectsExpiredV2Certificate(t *testing.T) {
	node, bitcoinChain, walletPublicKey := setupCovenantSignerTestNode(t)
	...
	// Issue and submit with the wall-clock counter still in place: the counter is
	// far below EndBlock (so the certificate is valid) and keeps advancing so the
	// covenant transaction signing triggered by OnSubmit completes.
	request := buildValidSelfV1CovenantRequest(t, node, bitcoinChain, walletPublicKey)
	submitResult, err := service.Submit(context.Background(), covenantsigner.TemplateSelfV1, covenantsigner.SignerSubmitInput{
		RouteRequestID: "ors_self_poll_expiry",
		Stage:          covenantsigner.StageSignerCoordination,
		Request:        request,
	})
	if err != nil {
	...
	// Freeze the host chain past the signed expiry. Poll performs no signing, so
	// a frozen counter is safe and drives the expiry rejection deterministically.
	blockCounter := &controllableBlockCounter{}
	blockCounter.setHeight(testSignerApprovalEndBlock + 1)
	node.chain.(*localChain).blockCounter = blockCounter

	_, err = service.Poll(context.Background(), covenantsigner.TemplateSelfV1, covenantsigner.SignerPollInput{
		RouteRequestID: "ors_self_poll_expiry",
	...
	if err == nil || !strings.Contains(err.Error(), "signer approval certificate has expired") {
		t.Fatalf("expected expiration rejection on poll, got %v", err)
	}
}
