// Smoke-test the ecdsa ESLint config to verify:
// (a) no-only-tests fires on it.only() in test files
// (b) ignored paths (typechain/**) produce zero real lint messages
// (c) ordinary test files are actually linted (not silently skipped)
//
...
// Checks (a) and (c) deliberately reuse the SAME test/ path, overwritten in
