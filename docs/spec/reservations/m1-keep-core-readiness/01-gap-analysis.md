...
| Re-anchor proof submission not wired to `WalletMovingFunds` trigger | Major | `pkg/maintainer/spv/reservation_wiring.go` has zero references to `WalletMovingFunds`, `OnWalletMovingFunds`, or `reanchor`. `submitReservationReanchorProof` (`reservation_reanchor_proof.go:49`) exists as a callable function but nothing in the wiring file invokes it on a wallet-rotation event — re-anchor is explicitly M1-in-scope (`roadmap.md` §0.2, "Re-anchor... Yes — and this is desirable"). | `feature-spec.md` §13 ("must initiate re-anchor promptly on `WalletMovingFunds(walletPubKeyHash)`, not wait for an expiry signal") |
