...
| `m1/bridge-integration-seams` | New work | Bridge integration seams: `Deposit.sol` reveal path, `Wallets.sol` lifecycle gates, `WalletProposalValidator.sol` (sweep-exclusion guard plus m1 proposal validators), `BridgeGovernance.sol` and `BridgeGovernanceParameters.sol` wiring, and `Bridge.sol`'s `isReservedDeposit`, `setReservationRouter` and `fallback`. Review focus must note that `WalletProposalValidator` is non-upgradeable, so its m1 content is a real decision rather than a free omission. **Owns the missing deploy step (added 2026-08-25):** a new `97_set_reservation_parameters.ts` (and any companion `setVaultStatus` activation step) that wires the freshly deployed `ReservationVault` into the Bridge on a clean deploy — `95_deploy_reservation_vault.ts` and `96_transfer_reservation_vault_ownership.ts` both end without doing so (95's NOTE block explicitly delegates the wiring to governance, 96 only transfers `Ownable` ownership). The new script must call `updateReservationCaps` first, then `updateReservationParameters`, to avoid the relational-check revert the bootstrap-ordering test in PR #B already exhibits (see `manager-to-implementor-2026-08-25-decision1-testrun.md` §2). Without this script the vault ships orphaned on a fresh deployment and reservations are unreachable. | +820 -50 | Seventh: integrates vault with bridge; depends on storage layout and router. |
| `m1/reanchor-dissolution-gate-fix` | New follow-up (2026-09-03 audit finding) | **Correction to `m1/reanchor-core`'s row above**, which claims "(unbounded, no dissolution gate)" — false as landed: `requestReservationReanchor`'s `block.timestamp < reservation.dissolutionEligibleAt` gate was never actually deleted (confirmed via `git log -L` on `Reservation.sol`: present since the first re-anchor extraction `8320906c`, unchanged through the `d600a8bf` 34-finding review pass and the `a5630007` test-coverage commit). This PR deletes the gate and rewrites the one test that exercises it (`Bridge.ReservationStrandingLibrary.test.ts` "rejects when reservation is dissolution-eligible"). Full spec: `m1-tbtc-v2-readiness/03-followup-pr-spec.md`. | +0 −7 (plus one test rewrite) | New: independent of A-H, based on current `reservations-upgrade` tip; no ordering dependency since it only removes a require statement. |
...
10. **Open PR #G**: `m1/bridge-integration-seams` -> epic branch
    - Implement Bridge integration seams: `Deposit.sol` reveal path, `Wallets.sol` lifecycle gates, `WalletProposalValidator.sol` (sweep-exclusion guard plus m1 proposal validators), `BridgeGovernance.sol` and `BridgeGovernanceParameters.sol` wiring, and `Bridge.sol`'s `isReservedDeposit`, `setReservationRouter` and `fallback`. **Owns the missing deploy step (added 2026-08-25):** a new `solidity/deploy/97_set_reservation_parameters.ts` (and any companion `setVaultStatus` activation step) that wires the freshly deployed `ReservationVault` into the Bridge on a clean deploy. `95_deploy_reservation_vault.ts` and `96_transfer_reservation_vault_ownership.ts` both end without doing so — `95`'s NOTE block explicitly delegates the wiring to governance, `96` only transfers `Ownable` ownership. Repo-wide grep across `solidity/` finds zero callers of `updateReservationParameters` / `updateReservationCaps` outside the contract definitions, the `BridgeGovernance` wrapper, and `typechain/` bindings, so on a fresh deploy today the vault ships orphaned and reservations are unreachable. The new script must call `updateReservationCaps` first (passes trivially because `reservationMaxTotalAmount` defaults to `0`), then `updateReservationParameters`, to avoid the relational-check revert the bootstrap-ordering test in PR #B already exhibits (see `agent-docs/m1/manager-to-implementor-2026-08-25-decision1-testrun.md` §2.b). Detail in §4.1 PR #G row.
