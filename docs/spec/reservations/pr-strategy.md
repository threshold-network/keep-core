...
| `m1/acceptance-core` | #1091 (acceptance half) + **rebased #1093** | Two-phase acceptance: request side (vault set, deposit revealed, wallet Live, nonce increment), proof side (SPV, consumeAcceptedDeposit, emit events), settlement path (writes `expiresAt`, `dissolutionEligibleAt`, `mintedAmount`, `anchorAmount`, `state=Active`, external mint). **Rebased #1093 required, not raw #1093**: #1093 reworks the backing-accounting model #1091 established (H-04, `inventory/pr-map.md` §5.3), but `Reservation.sol` (which this settlement path calls into) is also one of #1102's ten touched files (`registerReservation`/`resolveAcceptedDeposit`, `§7`) — raw #1093 inherits #1091's pre-fold staleness there. Only the rebased branch (§9 step 3) carries both. | +1800 -400 | Third: uses router and storage. First reachable path. |
| `m1/reanchor-core` | #1091 (re-anchor half) + **rebased #1093** | Re-anchor request and proof (unbounded, no dissolution gate), settlement path (wallet amount/count transfers, `walletPubKeyHash` update, `anchorAmount` rewrite, external in-kind fee call). **Rebased #1093 required, not raw #1093** — verified 2026-08-21: raw #1093 has the H-04 `mintedAmount = newAnchorAmount` rewrite (absent at #1091) but zero occurrences of `cumulativeReanchorFee` anywhere in `Reservation.sol`/`ReservationProofs.sol` — it is missing #1102's `maxCumulativeReanchorFee` grinding-cap enforcement inside `submitReservationReanchorProof` entirely, not just the storage field. Extracting raw #1093 ships the H-04 fix but silently drops the fee-cap; only the rebased branch (§9 step 3) has both. | +1000 -300 | Fourth: builds on acceptance; shares settlement helpers. |
...
3. **Rebase #1091 onto #1090, then #1092 onto the rebased #1091, then #1093 onto the rebased #1092** (sequentially, per §7 — never rebase a descendant directly onto #1090; that replays every intermediate branch's commits a second time):  
   `pre_rebase_settlement_tip=$(git rev-parse feat/utxo-reservation-settlement)` (capture all three tips before rebasing anything — needed by the later `--onto` commands)  
   `pre_rebase_renewal_tip=$(git rev-parse feat/utxo-reservation-renewal)`  
   ...
   `git checkout feat/utxo-reservation-backing`  
   `git rebase --onto feat/utxo-reservation-renewal $pre_rebase_renewal_tip`  
   If the rebase conflicts, resolve it, then run the reservation test suite on each rebased branch. Do not extract until all three suites pass. **The #1091-onto-#1090 step is where a real conflict is expected and must be resolved by hand, not mechanically**: #1102 independently reworks `submitReservationReanchorProof` to add `maxCumulativeReanchorFee`/`cumulativeReanchorFee` grinding-cap enforcement and to stop decrementing `reservationTotalAmount` on re-anchor (because the cap now tracks gross `mintedAmount`, which re-anchor never changes) — verified 2026-08-21 by reading `#1102`'s full diff to this function. Resolving the conflict means keeping both: #1091's two-phase settlement structure and #1102's cap enforcement plus accounting-basis fix. #1093's own re-anchor change (the H-04 `mintedAmount = newAnchorAmount` rewrite, `§4.1`) then rebases cleanly on top of the resolved #1091/#1092, because it touches adjacent lines in the same function rather than the ones #1102 changed.
4. **Open PR #A**: `m1/storage-layout` -> epic branch  
   - Extract storage layout, enums, structs, governance parameters from rebased #1088 (post-#1102), #1090, #1091, #1092, rebased #1093, #1094, and **#1096** (required for `ReservationAction.isPartial`, `retryCreditSourceNonce`, `BridgeState.Storage.reservationRetryCreditActionNonce` — §4.1). **Gap**: step 3 rebases #1091-#1093, not #1096 (which branches #1094->#1095->#1096, all downstream of the now-rebased #1093). #1094-#1096 still inherit staleness relative to their own history before this rewrite reaches them, so they do not carry the #1102 fold either. Before extracting #1096's three fields, either rebase #1094-#1096 sequentially onto the rebased #1093, or verify none of the three fields sit in a file #1102 touched (`Reservation.sol`, `BridgeState.sol`, etc., per §7).
...
6. **Open PR #C**: `m1/acceptance-core` -> epic branch  
   - Extract acceptance request/proof/settlement from **rebased #1093** (not raw #1093 — see step 3's conflict-resolution note; #1093 inherits the merged fold+H-04 form once rebased), verified via extraction provenance.
7. **Open PR #D**: `m1/reanchor-core` -> epic branch  
   - Extract re-anchor request/proof/settlement from **rebased #1093** (not raw #1093): raw #1093 has the H-04 `mintedAmount` rewrite but zero occurrences of `cumulativeReanchorFee` anywhere in `Reservation.sol`/`ReservationProofs.sol` (verified 2026-08-21) — it lacks #1102's fee-grinding cap entirely, not just the storage field (which §4.1's storage-layout row already sources from the fold). Only the rebased branch has both.
