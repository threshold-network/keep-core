...
| `beginWalletClosing`: reservation count precondition | `Wallets.sol:674-676` | #1094 | invariant | yes | yes | `require(self.walletReservationsCount[walletPubKeyHash] == 0, "Wallet still custodies reservations")`. A wallet with active reservations cannot enter Closing. |
...
| `finalizeWalletClosing`: reservation count precondition | `Wallets.sol:706-708` | #1094 | invariant | yes | yes | `require(self.walletReservationsCount[walletPubKeyHash] == 0, "Wallet still custodies reservations")`. Even in Closing, a late sweep can register, but a wallet with reservations cannot finalize. |
