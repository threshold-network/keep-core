...
NOTE: Running the `maintainer` command starts the client-info HTTP endpoint on
all interfaces on port `9601` by default. Set `ClientInfo.Port` to `0` in
configuration, or pass `--clientInfo.port=0`, to disable the endpoint and
these metrics.
